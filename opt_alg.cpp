   ...
   int Nmax, matrix ud1, matrix ud2) {
    try {
        ...
        while (iteration < Nmax) {
            ...
            // Sprawdzenie normy gradientu (warunek zakoñczenia)
            if (norm(d) < epsilon) {
                Xopt.flag = 1; // Minimum osi¹gniête
                break;
            }

            // Wyznaczanie d³ugoœci kroku h(i) metod¹ ekspansji i z³otego podzia³u
            ...
            double h = m2d(golden_result.x(0, 0)); // Optymalna d³ugoœæ kroku

            // Aktualizacja punktu x(i+1) = x(i) + h(i) * d(i)
            Xopt.x = Xopt.x + h * d;

            // Obliczanie wartoœci funkcji celu w nowym punkcie
        ...
        // Jeœli liczba iteracji przekroczy³a Nmax
        if (iteration >= Nmax) {
            Xopt.flag = -1; // Niepowodzenie w osi¹gniêciu minimum
        }
   ...
   int Nmax, matrix ud1, matrix ud2) {
    try {
        ...
        while (iteration < Nmax) {
            ...
            // Przechowywanie bie¿¹cych wartoœci gradientu i kierunku
            g_prev = Xopt.g;
            d_prev = d;
...
solution Newton(matrix(*ff)(matrix, matrix, matrix), matrix(*gf)(matrix, matrix, matrix),
                ...
                matrix ud2) {
    try {
        solution Xopt; // Obiekt, który przechowuje wynik optymalizacji
...
golden(matrix(*ff)(matrix, matrix, matrix), double a, double b, double epsilon, int Nmax, matrix ud1, matrix ud2) {
    try {
        ...
        // Pocz¹tkowe wartoœci przedzia³u i punktów
        double c = b - alpha * (b - a);
        double d = a + alpha * (b - a);

        // Obliczenie wartoœci funkcji celu w punktach c i d
        matrix xc = c;
        matrix xd = d;
        ...
        while ((b - a > epsilon) && (iteration < Nmax)) {
            if (fc < fd) {
                ...
                // Przeliczenie wartoœci funkcji celu w nowym punkcie
                xd = xc;
                fd = fc;
        ...
        // Wyznaczenie punktu minimalnego (œrodek przedzia³u [a, b])
        double x_opt = (a + b) / 2.0;
        Xopt.x = x_opt; // Wspó³rzêdna optymalna
        Xopt.y = solution().fit_fun(ff); // Wartoœæ funkcji w minimum
